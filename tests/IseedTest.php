...
class DatabaseTest extends TestCase {
 ...
 public function __construct() {
  ...
  static::$stubsDir     = __DIR__ . '/../src/Pittacusw/Database/stubs';
 ...
 public function testPopulatesStub() {
  ...
  $iSeed = new Pittacusw\Database\Database();
 ...
 public function testTableNotFoundException() {
  $this->expectException(\Pittacusw\Database\TableNotFoundException::class);
  $this->expectExceptionMessage('Table nonexisting was not found.');

  $hasTable = m::mock('Pittacusw\Database\Database[hasTable]')
               ->makePartial();
  $hasTable->shouldReceive('hasTable')
           ->once()
           ->andReturn(FALSE);
 ...
 public function testCanGetStubPath() {
  ...
  $expected = substr(__DIR__, 0, - 5) . 'src' . DIRECTORY_SEPARATOR . 'Pittacusw' . DIRECTORY_SEPARATOR . 'Database' . DIRECTORY_SEPARATOR . 'stubs';
  $this->assertEquals($expected, $output);
 }

 public function testCanGenerateSeed() {
  $file     = m::mock(\Illuminate\Filesystem\Filesystem::class)
               ->makePartial();
  $composer = m::mock(\Illuminate\Support\Composer::class, [$file])
               ->makePartial();
  $mocked   = m::mock(\Pittacusw\Database\Database::class, [
   $file,
   $composer
  ])
               ->makePartial();
  $mocked->shouldReceive('readStubFile')
         ->once()
         ->with(substr(__DIR__, 0, - 5) . 'src' . DIRECTORY_SEPARATOR . 'Pittacusw' . DIRECTORY_SEPARATOR . 'Database' . DIRECTORY_SEPARATOR . 'stubs' . DIRECTORY_SEPARATOR . 'seed.stub');
  $file->shouldReceive('put')
       ->once()
       ->with('seedPath', 'populatedStub');
  $mocked->shouldReceive('hasTable')
         ->once()
         ->andReturn(TRUE);
  $mocked->shouldReceive('getData')
         ->once()
         ->andReturn([]);
  $mocked->shouldReceive('generateClassName')
         ->once()
         ->andReturn('ClassName');
  $mocked->shouldReceive('getSeedPath')
         ->once()
         ->andReturn('seedPath');
  $mocked->shouldReceive('getPath')
         ->once()
         ->with('ClassName', 'seedPath')
         ->andReturn('seedPath');
  $mocked->shouldReceive('populateStub')
         ->once()
         ->andReturn('populatedStub');
  $mocked->shouldReceive('updateDatabaseSeederRunMethod')
         ->once()
         ->with('ClassName')
         ->andReturn(TRUE);
